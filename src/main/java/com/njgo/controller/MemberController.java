...
public class MemberController {
	...
	public String memberLogin(HttpSession session ,MemberDTO memberDTO,Model model){
		...
		if (memberDTO_result != null) {
			...
			int result =0;
			if(Integer.parseInt(memberDTO_result.getGrade())>1){
				result =messageService.messageCheck("운영자");
			}else{
				 result =messageService.messageCheck(memberDTO_result.getNickName());
			}
	...
	public void join(Model model, @RequestParam String login_mode)throws Exception{
