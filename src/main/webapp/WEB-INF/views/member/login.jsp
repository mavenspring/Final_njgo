		  ...
		  <div class="panel-body"> 
		    <form name="form_login" id="formLogin" method="post" action="memberLogin" autocomplete="off" onsubmit="return doSubmit()">
		      ...
		      <div class="form-group">
		        ...
		        <input type="hidden" name="path" class="form-control" id="path" value="${path}" >
...
<script>
	...
	function loginWithKakao() { 
		...
		//1. 로그인
		// Kakao.Auth.login()   : 한번 로그인했던 계정을 브라우저를 껏다가키지 않는한 자동로그인 해줌
		// Kakao.Auth.loginForm : 다른 계정으로 로그인을 할수있도록해줌 
	        	  ...
	        	  //2. 앱 연결 유무 확인
		       	  Kakao.API.request({
		       		...
		       		success: function(res){	 //앱에 가입되지 안앗음... 바로 회원가입 약관 페이지로 이동
		       		 location.href ="provision?login_mode=SNS_join"; 
		       		} ,
			 	     		...
			 	     		success: function(data){
			 	     			...
			 	     			$.ajax({
			 	     				...
			 	     				success : function(result) {
			 	     					...
			 	     					// 즉, 회원가입이 되있는 상태이므로 로그인 가능
									 	if(result.trim()==0){
									 		location.href="memberLogin?login_mode=SNS_join&kakaoID="+kakaoID+"&SNS_photo="+SNS_photo+"&path=${path}";
											/* $("#kakaoID").val(data.id);
											$("#btn_login").submit(); */
										} 
									 	if(result.trim()==1){
